...
function Radisk(opt){
	...
	// Use Deno.Kv if in Deno environment
	if (isDenoEnv && !opt.store) {
		if (globalThis.DenoKV) {
			opt.store = DenoKV(opt);
		}
	}
	...
	// Replace Node-specific fs operations with Deno's file API
	if (isDenoEnv && !opt.store.put) {
		opt.store.put = async function(file, data, cb) {
			try {
				const path = opt.file + '/' + ename(file);
				await Deno.mkdir(opt.file, { recursive: true });
		...
		opt.store.list = async function(cb) {
			try {
				await Deno.mkdir(opt.file, { recursive: true });
				for await (const entry of Deno.readDir(opt.file)) {
					if (entry.isFile) {
						cb(entry.name);
					}
				}
